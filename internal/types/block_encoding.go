...
// MarshalSSZ ssz marshals the MultiMessageAggregate object
func (m *MultiMessageAggregate) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(m)
}

// MarshalSSZTo ssz marshals the MultiMessageAggregate object to a target array
func (m *MultiMessageAggregate) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(4)
	...
	// Field (0) 'Proof'
	if size := len(m.Proof); size > 524288 {
		err = ssz.ErrBytesLengthFn("MultiMessageAggregate.Proof", size, 524288)
		return
	}
	dst = append(dst, m.Proof...)

	return
}

// UnmarshalSSZ ssz unmarshals the MultiMessageAggregate object
func (m *MultiMessageAggregate) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 4 {
		return ssz.ErrSize
	...
	// Field (0) 'Proof'
	{
		...
		if cap(m.Proof) == 0 {
			m.Proof = make([]byte, 0, len(buf))
		}
		m.Proof = append(m.Proof, buf...)
	}
	return err
}

// SizeSSZ returns the ssz encoded size in bytes for the MultiMessageAggregate object
func (m *MultiMessageAggregate) SizeSSZ() (size int) {
	size = 4
...
// HashTreeRoot ssz hashes the MultiMessageAggregate object
func (m *MultiMessageAggregate) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(m)
}

// HashTreeRootWith ssz hashes the MultiMessageAggregate object with a hasher
func (m *MultiMessageAggregate) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	indx := hh.Index()

	// Field (0) 'Proof'
	{
		elemIndx := hh.Index()
		byteLen := uint64(len(m.Proof))
		if byteLen > 524288 {
			err = ssz.ErrIncorrectListSize
			return
		}
...
// GetTree ssz hashes the MultiMessageAggregate object
func (m *MultiMessageAggregate) GetTree() (*ssz.Node, error) {
...
func (s *SignedBlock) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'Proof'
	{
		buf = tail[o1:]
		if s.Proof == nil {
			s.Proof = new(MultiMessageAggregate)
		}
